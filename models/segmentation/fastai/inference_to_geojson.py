...
if __name__ == "__main__":
    ...
    if len(arg) < 3:
        ...
        print('python inference_to_geojson.py path_to_images path_to_results')
        exit(0)
    ...
    for dirpath, dirnames, filenames in os.walk(dir_path):
      for filename in filenames:
        ...
        # if full_path.split('/')[-2]=='building':
        if filename.endswith('.tif'):
          # print(filename)
          test_tile = skimage.io.imread(full_path)
          result = get_pred(inference_learner, test_tile)
          mask2poly = sol.vector.mask.mask_to_poly_geojson(result,channel_scaling=[1,1,0],bg_threshold=245,simplify=True,tolerance=2)
          src = rio.open(full_path)
          result_polys = sol.vector.polygon.georegister_px_df(mask2poly,affine_obj=src.transform,crs=src.crs)
          geofile = filename.replace('.tif','.geojson')
          out_path = os.path.join(dst_dir,geofile)
          result_polys.to_file(out_path, driver='GeoJSON')



#python  inference_to_geojson.py /images /results 